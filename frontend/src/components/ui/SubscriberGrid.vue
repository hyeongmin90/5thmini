    ...
    <v-container>
        ...
        <div class="panel">
            <div class="gs-bundle-of-buttons" style="max-height:10vh;">
                ...
                <v-btn style="margin-left: 5px;" @click="joinMembership" class="contrast-primary-text" small color="primary" :disabled="!hasRole('sub')">
                ...
                <v-table>
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td class="whitespace-nowrap" label="ExpirationDate">{{ val.expirationDate }}</td>
                            <td class="whitespace-nowrap" label="Telecom">{{ val.telecom }}</td>
                            <td class="whitespace-nowrap" label="PhoneNumber">{{ val.phoneNumber }}</td>
                            <td class="whitespace-nowrap" label="IsRecommended">{{ val.isRecommended }}</td>
                ...
                <v-card>
                    ...
                    <v-card-text>
                        <div>
                            ...
                            <String label="Subscription" v-model="selectedRow.subscription" :editMode="true"/>
                            <String label="Password" v-model="selectedRow.password" :editMode="true"/>
                            <Date label="ExpirationDate" v-model="selectedRow.expirationDate" :editMode="true"/>
                            ...
                            <Boolean label="IsRecommended" v-model="selectedRow.isRecommended" :editMode="true"/>
...
export default {
    ...
    methods:{
        async joinMembership(){
            try{
                var path = "joinMembership".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, null)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','join membership 성공적으로 처리되었습니다.')
